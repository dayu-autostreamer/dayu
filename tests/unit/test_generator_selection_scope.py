...
def test_physical_topology_accepts_source_device_outside_processing_node_set(monkeypatch):
    ...
    pytest.importorskip("torch_geometric", reason="Hedger topology tests require torch_geometric in the test environment")
...
def test_hedger_initial_deployment_policy_returns_processing_plan_when_source_is_external(monkeypatch):
    ...
    pytest.importorskip("torch_geometric", reason="Hedger policy tests require torch_geometric in the test environment")
