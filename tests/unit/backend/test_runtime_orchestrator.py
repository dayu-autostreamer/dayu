...
class FakeCluster:
    ...
    def runtime_metrics(self, refs, node_inventory=None):
        self.metric_calls.append((copy.deepcopy(refs), copy.deepcopy(node_inventory)))
        return {}
...
def test_install_activates_scheduler_first_then_binds_observed_hash_and_endpoint_uids():
    ...
    assert sessions.saved_phases == [
        "activating-scheduler", "activating-runtime", "publishing", "active",
    ]
...
def test_runtime_metrics_uses_session_pod_uids_and_cached_inventory_without_extra_node_refresh():
    orchestrator, cluster, _, _, _, _ = make_orchestrator(
        {"detect": ["edge-a"]},
    )
    directory = install(orchestrator, "detect")
    ...
    assert orchestrator.runtime_metrics("detect") == {}

    assert cluster.inventory_calls == inventory_calls
    refs, snapshot = cluster.metric_calls[-1]
    processor = next(
