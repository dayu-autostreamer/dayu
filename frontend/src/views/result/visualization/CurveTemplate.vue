...
export default {
	...
	props: {
		...
		data: {
			...
			required: true,
			validator: (value) => {
				return Array.isArray(value) && value.every((item) => item.taskId !== undefined);
			},
		},
		variableStates: {
			type: Object,
	...
	setup(props) {
		...
		const showEmptyState = computed(() => {
			...
			const hasValidData = hasData && activeVariables.value.some((v) => safeData.value.some((d) => d[v] !== undefined));
				...
				axisLabel: {
					formatter: (value) => {
						...
						if (valueTypes.value[activeVariables.value[0]] === 'category') {
							const entry = Object.entries(discreteValueMap.value[activeVariables.value[0]]).find(
								([, v]) => v === value
							);
			...
			const seriesConfig = activeVariables.value.map((varName) => {
				...
				return {
					...
					data: values.map((v) => {
						...
						return valueTypes.value[varName] === 'category' ? getDiscreteValue(varName, v) : Number(v);
					}),
					// 新增：空数据跳过渲染
					connectNulls: false,
					// 新增：优化渲染性能
