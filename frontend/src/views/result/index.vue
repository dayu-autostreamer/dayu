	...
	<div class="home-container layout-pd">
		...
		<el-row :gutter="15" class="home-card-two mb15">
			<el-col :xs="24" :sm="24" :md="20" :lg="20" :xl="20">
				<div class="home-card-item data-source-container" :class="{ 'source-loading': isSourceLoading }">
					<div class="flex-margin flex w100">
						<div class="flex-auto" style="font-weight: bold">
							...
							<input ref="uploadInput" type="file" hidden @change="handleFileUpload" />
						</div>
			...
			<el-col :xs="24" :sm="24" :md="4" :lg="4" :xl="4">
				<div class="home-card-item export-container">
					<el-button type="primary" class="export-button" @click="exportTaskLog"> Export Result Log </el-button>
				</div>
			</el-col>
		</el-row>
	...
	methods: {
		...
		processVizData(vizConfig) {
			...
			const filteredData = this.bufferedTaskCache[sourceId]
				.filter((task) => {
					return task.data?.some(
						(item) => validVizIds.has(String(item.id)) && String(item.id) === String(vizConfig.id)
					);
				})
				.map((task) => {
