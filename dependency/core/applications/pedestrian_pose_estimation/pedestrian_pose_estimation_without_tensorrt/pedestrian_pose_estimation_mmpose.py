...
class PedestrianPoseEstimation:
    service_name = 'pedestrian-pose-estimation'
    ...
    def _fallback_skeletons(people):
        ...
        for index, detection in enumerate(people):
            ...
            skeletons.append({
                ...
                'source_object_id': detection.get('object_id'),
                'frame_id': detection.get('frame_id', detection.get('frame_index', 0)),
                'label': PedestrianPoseEstimation._label(detection) or 'pedestrian',
                'category': detection.get('category', PedestrianPoseEstimation._label(detection) or 'pedestrian'),
                'bbox': detection.get('bbox', []),
                'keypoints': keypoints,
                'orientation': 'toward-road',
            })
    ...
    def _filter_detections(detections, categories):
        ...
        return [detection for detection in detections if PedestrianPoseEstimation._label(detection) in category_set]
