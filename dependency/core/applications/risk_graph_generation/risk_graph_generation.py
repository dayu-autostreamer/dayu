    ...
    def __init__(self, trt_weights='', trt_plugin_library='', non_trt_weights='', device=0):
        ...
        if use_tensorrt:
            ...
            if jetpack_version == 6:
                ...
                from .risk_graph_generation_with_tensorrt import RiskGraphGenerationTensorRT10
                self.model = RiskGraphGenerationTensorRT10(
                ...
                from .risk_graph_generation_with_tensorrt import RiskGraphGenerationTensorRT8
                self.model = RiskGraphGenerationTensorRT8(
                    weights=self.trt_weights,
                    plugin_library=self.trt_plugin_library,
                    device=self.device,
                )
        else:
            from .risk_graph_generation_without_tensorrt import RiskGraphGenerationWithoutTensorRT
            self.model = RiskGraphGenerationWithoutTensorRT(weights=self.non_trt_weights, device=self.device)
