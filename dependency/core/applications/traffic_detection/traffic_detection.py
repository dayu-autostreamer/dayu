    ...
    def __init__(self, trt_weights='', trt_plugin_library='', non_trt_weights='', device=0, confidence_threshold=0.25):
        ...
        if use_tensorrt:
            ...
            if jetpack_version == 6:
                ...
                from .traffic_detection_with_tensorrt import TrafficDetectionTensorRT10
                self.model = TrafficDetectionTensorRT10(
                ...
                from .traffic_detection_with_tensorrt import TrafficDetectionTensorRT8
                self.model = TrafficDetectionTensorRT8(
                    weights=self.trt_weights,
                    plugin_library=self.trt_plugin_library,
                    device=self.device,
                )
        else:
            from .traffic_detection_without_tensorrt import TrafficDetectionWithoutTensorRT
            self.model = TrafficDetectionWithoutTensorRT(
