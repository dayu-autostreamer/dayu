    ...
    def __init__(self, trt_weights='', trt_plugin_library='', non_trt_weights='', device=0):
        ...
        if use_tensorrt:
            ...
            if jetpack_version == 6:
                ...
                from .vehicle_reidentification_tracking_with_tensorrt import VehicleReidentificationTrackingTensorRT10
                self.model = VehicleReidentificationTrackingTensorRT10(
                ...
                from .vehicle_reidentification_tracking_with_tensorrt import VehicleReidentificationTrackingTensorRT8
                self.model = VehicleReidentificationTrackingTensorRT8(
                    weights=self.trt_weights,
                    plugin_library=self.trt_plugin_library,
                    device=self.device,
                )
        else:
            from .vehicle_reidentification_tracking_without_tensorrt import VehicleReidentificationTrackingWithoutTensorRT
            self.model = VehicleReidentificationTrackingWithoutTensorRT(weights=self.non_trt_weights, device=self.device)
