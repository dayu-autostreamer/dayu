    ...
    def __init__(self, trt_weights='', trt_plugin_library='', non_trt_weights='', device=0):
        ...
        if use_tensorrt:
            ...
            if jetpack_version == 6:
                ...
                from .vehicle_attribute_recognition_with_tensorrt import VehicleAttributeRecognitionTensorRT10
                self.model = VehicleAttributeRecognitionTensorRT10(
                ...
                from .vehicle_attribute_recognition_with_tensorrt import VehicleAttributeRecognitionTensorRT8
                self.model = VehicleAttributeRecognitionTensorRT8(
                    weights=self.trt_weights,
                    plugin_library=self.trt_plugin_library,
                    device=self.device,
                )
        else:
            from .vehicle_attribute_recognition_without_tensorrt import VehicleAttributeRecognitionWithoutTensorRT
            self.model = VehicleAttributeRecognitionWithoutTensorRT(weights=self.non_trt_weights, device=self.device)
