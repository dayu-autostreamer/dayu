    ...
    def __init__(self, trt_weights='', trt_plugin_library='', non_trt_weights='', device=0):
        ...
        if use_tensorrt:
            ...
            if jetpack_version == 6:
                ...
                from .pedestrian_cyclist_intent_recognition_with_tensorrt import PedestrianCyclistIntentRecognitionTensorRT10
                self.model = PedestrianCyclistIntentRecognitionTensorRT10(
                ...
                from .pedestrian_cyclist_intent_recognition_with_tensorrt import PedestrianCyclistIntentRecognitionTensorRT8
                self.model = PedestrianCyclistIntentRecognitionTensorRT8(
                    weights=self.trt_weights,
                    plugin_library=self.trt_plugin_library,
                    device=self.device,
                )
        else:
            from .pedestrian_cyclist_intent_recognition_without_tensorrt import PedestrianCyclistIntentRecognitionWithoutTensorRT
            self.model = PedestrianCyclistIntentRecognitionWithoutTensorRT(weights=self.non_trt_weights, device=self.device)
