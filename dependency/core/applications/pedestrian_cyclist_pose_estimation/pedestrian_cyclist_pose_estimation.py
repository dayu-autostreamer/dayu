...
class PedestrianCyclistPoseEstimation:
    def __init__(self, trt_weights='', trt_plugin_library='', non_trt_weights='', device=0, non_trt_config=''):
        ...
        self.device = device
        self.non_trt_config = Context.get_file_path(non_trt_config) if non_trt_config else ''

        if use_tensorrt:
            jetpack_version = Context.get_parameter('JETPACK', direct=False)
            if jetpack_version == 6:
                LOGGER.info('Using TensorRT 10 (JetPack 6)')
                from .pedestrian_cyclist_pose_estimation_with_tensorrt import PedestrianCyclistPoseEstimationTensorRT10
                self.model = PedestrianCyclistPoseEstimationTensorRT10(
                ...
                from .pedestrian_cyclist_pose_estimation_with_tensorrt import PedestrianCyclistPoseEstimationTensorRT8
                self.model = PedestrianCyclistPoseEstimationTensorRT8(
                    weights=self.trt_weights,
                    plugin_library=self.trt_plugin_library,
                    device=self.device,
                )
        else:
            from .pedestrian_cyclist_pose_estimation_without_tensorrt import PedestrianCyclistPoseEstimationWithoutTensorRT
            self.model = PedestrianCyclistPoseEstimationWithoutTensorRT(
                weights=self.non_trt_weights,
                device=self.device,
                model_config=self.non_trt_config,
            )
