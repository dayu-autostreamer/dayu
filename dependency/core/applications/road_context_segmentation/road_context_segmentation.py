    ...
    def __init__(self, trt_weights='', trt_plugin_library='', non_trt_weights='', device=0):
        ...
        if use_tensorrt:
            ...
            if jetpack_version == 6:
                ...
                from .road_context_segmentation_with_tensorrt import RoadContextSegmentationTensorRT10
                self.model = RoadContextSegmentationTensorRT10(
                ...
                from .road_context_segmentation_with_tensorrt import RoadContextSegmentationTensorRT8
                self.model = RoadContextSegmentationTensorRT8(
                    weights=self.trt_weights,
                    plugin_library=self.trt_plugin_library,
                    device=self.device,
                )
        else:
            from .road_context_segmentation_without_tensorrt import RoadContextSegmentationWithoutTensorRT
            self.model = RoadContextSegmentationWithoutTensorRT(weights=self.non_trt_weights, device=self.device)
