...
from .adamec import AdaMECPolicySearch
...
@ClassFactory.register(ClassType.SCH_AGENT, alias='adamec')
class AdaMECAgent(BaseAgent, abc.ABC):
    def __init__(self, system, agent_id: int, sch_param: dict, baseline_param: dict):
        ...
        self.overhead_estimator = OverheadEstimator('adamec', 'scheduler/adamec', agent_id=agent_id)
    ...
    def update_task(self, task: Task):
        if task is None:
            LOGGER.debug('[AdaMEC] New task is None.')
            return
    ...
    def ensure_runtime_shape(self, info):
        ...
        # The partition index addresses business services plus the terminal
        # entry. Branching DAGs are rejected before policy search starts.
        current_services = [
            entry['service_name']
        ...
        if self.service_names is not None:
            if current_services != self.service_names:
                raise ValueError(
                    'AdaMEC does not support changing the pipeline topology at runtime'
                )
            return
    ...
    def ensure_search_engine(self, info):
        ...
        self.search_engine = AdaMECPolicySearch(
            ...
            adamec_param=self.baseline_param.get('adamec_param', {}),
