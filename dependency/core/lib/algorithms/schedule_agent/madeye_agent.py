...
from .madeye import MadEyePolicySearch


__all__ = ('MadEyeAgent',)


@ClassFactory.register(ClassType.SCH_AGENT, alias='madeye')
class MadEyeAgent(BaseAgent, abc.ABC):
    def __init__(self, system, agent_id: int, sch_param: dict, baseline_param: dict):
        ...
        self.overhead_estimator = OverheadEstimator('madeye', 'scheduler/madeye', agent_id=agent_id)
    ...
    def update_task(self, task: Task):
        if task is None:
            LOGGER.debug('[MadEye] New task is None.')
            return
    ...
    def ensure_runtime_shape(self, info):
        ...
        if self.service_names is not None:
            if current_services != self.service_names:
                raise ValueError(
                    'MadEye does not support changing the pipeline topology at runtime'
                )
            return
    ...
    def ensure_search_engine(self, info):
        ...
        self.search_engine = MadEyePolicySearch(
            ...
            madeye_param=self.baseline_param.get('madeye_param', {}),
