...
def _load_mapping(value, label):
    ...
    if isinstance(value, str):
        ...
        if isinstance(loaded, dict):
            return loaded
    raise TypeError(f"IBDASH {label} must be a mapping or mounted file path")


def _samples(value, allow_zero=False):
    ...
    for item in raw:
        try:
            ...
            continue
        if math.isfinite(item) and (item >= 0.0 if allow_zero else item > 0.0):
            result.append(item)
...
class _IBDASHLatencyProfile:
    """IBDASH-owned immutable view of its offline timing profile."""
    ...
    def __init__(self, configuration, profile):
        ...
        if self.profile:
            ...
            if self.context["configuration"] != expected:
                raise ValueError(
                    "IBDASH latency profile context mismatch for configuration"
                )
    ...
    def _normalize_configuration(configuration):
        if not isinstance(configuration, dict):
            raise TypeError("IBDASH profile configuration must be a mapping")
        try:
            ...
            raise ValueError(
                "IBDASH profile configuration must be JSON serializable"
            ) from exc

    @staticmethod
    def _normalize_deployment(deployment):
        if not isinstance(deployment, dict):
            raise TypeError("IBDASH profile deployment must be a mapping")
        normalized = {}
    ...
    def _normalize_dag(dag):
        if not isinstance(dag, dict):
            raise TypeError("IBDASH profile DAG must be a mapping")
        normalized = {}
    ...
    def _normalize_context(cls, context):
        if not isinstance(context, dict):
            raise ValueError("IBDASH latency profile has no valid context")
        missing = [
        ...
        if missing:
            raise ValueError(
                "IBDASH latency profile context is incomplete: "
                + ", ".join(missing)
            )
        return {
            ...
            for service, devices in store.items():
                ...
                for device in devices:
                    if str(device) not in deployment.get(str(service), []):
                        raise ValueError(
                            "IBDASH latency profile contains data outside its "
    ...
    def ensure_context(self, configuration, deployment, dag):
        ...
        if self.context and self.context != expected:
            ...
            raise ValueError(
                "IBDASH latency profile context mismatch for "
                + ", ".join(mismatches)
            )
        ...
        for service in service_names(dag):
            ...
            if not devices:
                raise ValueError(
                    f"IBDASH fixed deployment has no replica for {service}"
                )
            for device in devices:
        ...
        if missing:
            raise ValueError(
                "IBDASH latency profile does not cover the active fixed "
    ...
    def estimate_overhead(self, channel, service, device, quantile):
        store = self.profile.get(channel) or {}
        store = store if isinstance(store, dict) else {}
        devices = store.get(str(service)) or {}
        devices = devices if isinstance(devices, dict) else {}
        values = _samples(devices.get(str(device)), allow_zero=True)
        if not values:
            values = [
                item for value in devices.values()
                for item in _samples(value, allow_zero=True)
            ]
        if not values:
            values = [
                item
                for candidate_devices in store.values()
                if isinstance(candidate_devices, dict)
                for value in candidate_devices.values()
                for item in _samples(value, allow_zero=True)
            ]
        return _quantile(values, quantile)
...
class IBDASHAgent(BaseAgent, abc.ABC):
    ...
    def __init__(
        ...
        self.latency_model = _IBDASHLatencyProfile(
            self.configuration,
            ...
            for device in candidates:
                ...
                for predecessor in dag[service].get("prev_nodes", []):
                    ...
                    if predecessor_device and predecessor_device != device:
                        ready += self.latency_model.estimate_overhead(
                            "transfer_pairs", service, device,
                            self.profile_quantile,
                        )
                    predecessor_finish.append(ready)
                ready = max(predecessor_finish, default=0.0)
                ready += self.latency_model.estimate_overhead(
                    "control_pairs", service, device,
                    self.profile_quantile,
                )
                ready += self.latency_model.estimate_overhead(
                    "dispatch_pairs", service, device,
                    self.profile_quantile,
                )
    ...
    def get_schedule_plan(self, info):
        with self.overhead_estimator, self._lock:
            ...
            if (
                ...
                LOGGER.info(
                    "[IBDASH] count=%s source=%s "
                    "projected_finish=%.4fs plan=%s",
    ...
    def update_task(self, task):
        # Keep the offline timing profile frozen during evaluation.
        pass
