...
__all__ = ('TopologyEncoders', 'NoGraphTopologyEncoders')
...
class NoGraphTopologyEncoders(nn.Module):
    """
    Encoder ablation that keeps node-level static and temporal features, while
    ...
    def __init__(self, d_model: int = 64, num_roles: int = 2, role_emb_dim: int = 8,
        ...
        self.physical = NoGraphPhysicalEncoder(d_model=d_model, num_roles=num_roles, role_emb_dim=role_emb_dim,
                                               dropout=dropout)
...
class NoGraphLogicalEncoder(nn.Module):
    """
    Logical encoder ablation:
      - keeps service static features and temporal histories
      - removes DAG structure features and GAT message passing
    """
...
class NoGraphPhysicalEncoder(nn.Module):
    """
    Physical encoder ablation:
      - keeps device static features, role identity, and temporal histories
