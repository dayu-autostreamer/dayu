...
class _DeploymentBackbonePPO(nn.Module):
    ...
    def _deployment_plan_level_terms(
        ...
        hard_mass_tau = min(max(float(mass_tau), 1.0e-3), 0.08)
        hard_mass_margin = 2.0 * hard_mass_tau
        hard_threshold_mass_prob = torch.where(
            edge_allowed,
            torch.sigmoid((select_logits.to(dtype=dtype) - hard_mass_margin) / hard_mass_tau),
            torch.zeros_like(select_logits, dtype=dtype),
        )
