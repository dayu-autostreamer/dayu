...
class HedgerDeploymentOnly(HedgerHeuristicOffloadingMixin, Hedger):
    """Use learned Hedger deployment while replacing offloading PPO with a heuristic."""

    def _current_offloading_rollout_agent(self):
        return None

    def inference_hedger(self):
        if not self.inference_cfg.run_deployment_worker:
            raise RuntimeError(
                "[HedgerDeploymentOnly][Inference] inference.run_deployment_worker must be true because "
                "the deployment policy is the learned component in this ablation."
            )
        if self.inference_cfg.run_offloading_worker:
            LOGGER.info(
                "[HedgerDeploymentOnly][Inference] Disable learned offloading worker; "
                "the schedule agent serves heuristic offloading decisions."
            )
            self.inference_cfg = replace(self.inference_cfg, run_offloading_worker=False)
