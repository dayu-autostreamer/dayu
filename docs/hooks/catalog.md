...
This catalog lists the registered hook implementations currently present in the repository. Alias names are the values that appear in templates, environment variables, and visualization configs.
...
- When a hook is experimental, prototype-only, or incomplete, keep that note in the catalog so operators do not mistake it for a fully productionized default.
