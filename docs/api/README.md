...
After task-aware scheduling creates a reservation, Generator either materializes the Task and acquires its
revision/root-task lease before submission, or cancels the exact reservation before ownership transfer. Generator,
Controller, Processor, and Distributor transfer ownership with an exact `{accepted, task_uuid}` acknowledgement; a
missing or mismatched ACK retains ownership at the sender. Controller and processor follow the exact routes in the Task
without calling the lease API. Distributor performs the single final renewal before durable persistence and then
completes Scheduler feedback and lease release. Exact controller/processor identities travel in the serialized Task;
no runtime API performs Kubernetes discovery. Scheduler extensions consume the separate in-process `LIVE` or
`COMMITTED` scheduling snapshot contract documented in the runtime API reference.
